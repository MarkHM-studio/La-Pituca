...
public class TipoPagoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<TipoPagoResponse> obtenerPorId(@PathVariable Long id){
        TipoPagoResponse tipoPagoResponse = tipoPagoService.obtenerPorId(id);
        return ResponseEntity.ok(tipoPagoResponse);
    }

    @PutMapping("/{id}")
    public ResponseEntity<TipoPagoResponse> actualizar(@PathVariable Long id, @Valid @RequestBody TipoPagoRequest request){
